...
@torch.no_grad()
def generate_with_net(args, net, device, class_idx=None):
    assert args.num_images_per_class is not None, "you have to specify num_images_per_class!"
    num_images = args.num_images_per_class
    num_batches = (num_images - 1) // args.max_batch_size + 1
    all_batches = torch.as_tensor(np.arange(num_images)).tensor_split(num_batches)
    ...
    # generate images
    print(f'Generating {num_images} images to "{args.outdir}"...')
    for batch_seeds in tqdm(all_batches, unit='batch'):
        ...
        if  class_idx is not None: # use classifier guidance generation, otherwise random
            class_labels[:, :] = 0
...
def generate(args):
    ...
    print(f'cf_scale: {args.cfg_scale}')
    ...
    print(f"{args.model_type} (use_decoder: {args.use_decoder}) Model Parameters: {sum(p.numel() for p in net.parameters()):,}")

    # Load checkpoints
    ckpt = torch.load(args.ckpt_path, map_location=device)
    net.load_state_dict(ckpt['ema'])
    print(f'Load weights from {args.ckpt_path}')
